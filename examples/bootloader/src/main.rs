...
use embassy_boot_ch32::{AlignedBuffer, BlockingFirmwareUpdater, FirmwareUpdaterConfig};
use embassy_boot_ch32::{
    BootLoader, BootLoaderConfig, CoarseFlash, FLASH_BASE, State, active_size, active_start,
    dfu_size, dfu_start, state_size, state_start,
};
...
pub(crate) type BootFlash = CoarseFlash<Flash<'static, FlashBlocking>, COARSE_ERASE_SIZE>;
...
/// expects.
pub(crate) type FlashMutex = Mutex<NoopRawMutex, RefCell<BootFlash>>;
...
/// The clock setup for this build.
///
/// USB needs a 48 MHz USB clock, which these parts can only derive from a PLL
/// running at 48, 96 or 144 MHz, so a USB build runs at 144 MHz from the
/// internal oscillator. HSI rather than HSE so that the example works on a board
...
fn main() -> ! {
    ...
    // Moved out of the peripheral set here, but only consumed once a DFU session
    // is entered, so a board that merely boots never initialises the controller.
    #[cfg(usb_usbd)]
    let usb = usb_dfu::UsbPeripherals {
        usb: p.USBD,
        dp: p.PA12,
        dm: p.PA11,
    };
    ...
    loop {
        ...
        let loader = match BootLoader::try_prepare::<_, _, _, COPY_BUFFER_SIZE>(boot_config) {
            ...
            Err(e) => {
                ...
                // `session()` diverges, so this arm diverges as well and no
